...
export async function GET(request: Request) {
  try {
    ...
    for (let redirect = 0; redirect < 4; redirect += 1) {
      response = await fetch(current, { redirect: 'manual', headers: { Accept: 'image/avif,image/webp,image/png,image/jpeg,image/svg+xml,image/*', 'User-Agent': 'Anima Hidden Visual Reference/2.0' } });
